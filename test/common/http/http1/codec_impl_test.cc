...
TEST_F(Http1ClientConnectionImplTest, DoubleChunkedIsRejected) {
  initialize();
  NiceMock<MockResponseDecoder> response_decoder;
  Http::RequestEncoder& request_encoder = codec_->newStream(response_decoder);
  TestRequestHeaderMapImpl headers{{":method", "GET"}, {":path", "/"}, {":authority", "host"}};
...
TEST_F(Http1ClientConnectionImplTest, DoubleChunkedBody1) {
  TestScopedRuntime scoped_runtime;
  Runtime::LoaderSingleton::getExisting()->mergeValues(
...
TEST_F(Http1ClientConnectionImplTest, DoubleChunkedBody2) {
  TestScopedRuntime scoped_runtime;
  Runtime::LoaderSingleton::getExisting()->mergeValues(
      {{"envoy.reloadable_features.reject_unsupported_transfer_encodings", "false"}});
  initialize();
