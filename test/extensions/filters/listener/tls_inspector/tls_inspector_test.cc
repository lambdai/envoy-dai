...
public:
  TlsInspectorTest()
      : cfg_(std::make_shared<Config>(store_, std::chrono::milliseconds(0))),
...
TEST_F(TlsInspectorTest, MaxClientHelloSize) {
  EXPECT_THROW_WITH_MESSAGE(
      Config(store_, std::chrono::milliseconds(0), Config::TLS_MAX_CLIENT_HELLO + 1),
...
TEST_F(TlsInspectorTest, ClientHelloTooBig) {
  ...
  cfg_ = std::make_shared<Config>(store_, std::chrono::milliseconds(0), max_size);
