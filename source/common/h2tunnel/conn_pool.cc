#include "source/common/h2tunnel/conn_pool.h"
...
ConnPoolImpl::ConnPoolImpl(Event::Dispatcher& dispatcher, Upstream::HostConstSharedPtr host,
  ...
  if (host->address()->envoyInternalAddress()) {
    ENVOY_LOG(
        debug, "create new conn pool to host {} with transport_socket_options {}",
        host->address()->asStringView(),
        absl::StrCat(
            "pp dst = ",
            transport_socket_options_->proxyProtocolOptions()->dst_addr_ == nullptr
                ? "nullptr"
                : transport_socket_options_->proxyProtocolOptions()->dst_addr_->asStringView(),
            "src = ",
            transport_socket_options_->proxyProtocolOptions()->src_addr_ == nullptr
                ? "nullptr"
                : transport_socket_options_->proxyProtocolOptions()->src_addr_->asStringView()));
  }
}
...
void ConnPoolImpl::assignConnection(ActiveConn& conn, Tcp::ConnectionPool::Callbacks& callbacks) {
...
ConnPoolImpl::newConnection(Tcp::ConnectionPool::Callbacks& callbacks) {
  ...
  if (host_->cluster().resourceManager(priority_).pendingRequests().canCreate()) {
    ...
    callbacks.onPoolFailure(Tcp::ConnectionPool::PoolFailureReason::Overflow, nullptr);
...
void ConnPoolImpl::onConnectionEvent(ActiveConn& conn, Network::ConnectionEvent event) {
  if (event == Network::ConnectionEvent::RemoteClose ||
    ...
    if (conn.wrapper_ != nullptr) {
      ...
      if (conn.timed_out_) {
        reason = Tcp::ConnectionPool::PoolFailureReason::Timeout;
...
void ConnPoolImpl::onPendingRequestCancel(PendingRequest& request,
                                          Tcp::ConnectionPool::CancelPolicy cancel_policy) {
  ...
  if (cancel_policy == Tcp::ConnectionPool::CancelPolicy::CloseExcess &&
...
void ConnPoolImpl::ConnectionWrapper::addUpstreamCallbacks(
    Tcp::ConnectionPool::UpstreamCallbacks& cb) {
  ASSERT(!released_);
  callbacks_ = &cb;
}
...
void ConnPoolImpl::ActiveConn::onEvent(Network::ConnectionEvent event) {
  Tcp::ConnectionPool::UpstreamCallbacks* cb = nullptr;
  if (wrapper_ != nullptr && wrapper_->callbacks_ != nullptr) {
