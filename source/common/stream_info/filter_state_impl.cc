...
void FilterStateImpl::setData(absl::string_view data_name, std::shared_ptr<Object> data,
  ...
  if (parent_ && parent_->hasDataWithName(data_name)) {
    // TODO(lambdai): consider using different names or figure out why same name on different life
    // span is not allowed.
    FANCY_LOG(
        warn,
        "FilterState::setData<T> called twice with conflicting life_span on the same data_name {}",
        data_name);
    // throw EnvoyException(
    //     "FilterState::setData<T> called twice with conflicting life_span on the same
    //     data_name.");
  }
